...
mod tests {
    ...
    fn it_works() {
        ...
        assert_eq!(
            format!("{}", "red".red().bold()),
            "\x1b[1m\x1b[31mred\x1b[m\x1b[m"
        );
        assert_eq!(
            format!("{:?}", array.override_line()),
            "\x1b[2K\x1b[1G[1, 2, 3]\x1b[m"
        );
        let string = String::from("truecolor");
        assert_eq!(
            format!("{}", string.bg_truecolor(255, 255, 0)),
            "\x1b[48;2;255;255;0mtruecolor\x1b[m"
        );
    }
}
